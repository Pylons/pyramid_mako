...
from pyramid_mako.compat import text_, text_type
...
class TestMakoRendererFactory(Base, unittest.TestCase):
    ...
    def test_asset_spec_subfolder_filenames(self):
        info = self._makeRendererInfo("tests:fixtures/helloworld.mak")
        renderer = self._callFUT(info)
        self.assertEqual(renderer.template.path, "tests:fixtures/helloworld.mak")
        self.assertTrue(renderer.defname is None)

    def test_asset_spec_subfolder_filenames_with_def(self):
        info = self._makeRendererInfo("tests:fixtures/helloworld#def.mak")
        renderer = self._callFUT(info)
        self.assertEqual(renderer.template.path, "tests:fixtures/helloworld.mak")
        self.assertEqual(renderer.defname, "def")
...
class TestIntegrationWithPreprocessorSettings(unittest.TestCase):
    def setUp(self):
        self.config = testing.setUp()
        self.settings = {
            "mako.directories": "tests:fixtures",
            "mako.preprocessor": "tests.dummy_mako_preprocessor",
...
class Test_parse_options_from_settings(Base, unittest.TestCase):
    def _callFUT(self, settings, settings_prefix="mako."):
        ...
        return parse_options_from_settings(
            settings, settings_prefix, self.config.maybe_dotted
        )
    ...
    def test_directories_list(self):
        ...
        result = self._callFUT(settings)
        module_path = os.path.dirname(sys.modules["__main__"].__file__).rstrip(
            "."
        )  # ./setup.py
        self.assertEqual(
            result["directories"],
            [os.path.join(module_path, "a"), os.path.join(module_path, "b")],
        )

    def test_with_module_directory_asset_spec(self):
        module_directory = "tests:fixtures"
        settings = {
            "mako.directories": self.templates_dir,
    ...
    def test_multiple_registration_different_name(self):
        ...
        self.assertEqual(
            result["directories"],
            [os.path.join(module_path, "c"), os.path.join(module_path, "d")],
        )
...
class TestIntegrationWithDirectories(unittest.TestCase):
    def setUp(self):
        self.config = testing.setUp()
        self.config.add_settings({"mako.directories": "tests:fixtures"})
        self.config.include("pyramid_mako")
    ...
    def test_render_to_response(self):
        ...
        self.assertEqual(
            result.ubody.replace("\r", ""), text_("\nHello föö\n", "utf-8")
        )

    def test_render_to_response_pkg_spec(self):
        from pyramid.renderers import render_to_response

        result = render_to_response("tests:fixtures/helloworld.mak", {"a": 1})
        self.assertEqual(
            result.ubody.replace("\r", ""), text_("\nHello föö\n", "utf-8")
        )
    ...
    def test_template_not_found(self):
        ...
        self.assertRaises(
            TemplateLookupException, render, "helloworld_not_here.mak", {}
        )
    ...
    def test_add_mako_renderer(self):
        ...
        self.config.add_settings({"foo.directories": "tests:fixtures"})
        self.config.add_mako_renderer(".foo", settings_prefix="foo.")
        result = render("nonminimal.foo", {"name": "<b>fred</b>"}).replace("\r", "")
        self.assertEqual(result, text_("Hello, &lt;b&gt;fred&lt;/b&gt;!\n"))
    ...
    def test_add_mako_renderer_before_settings(self):
        ...
        config.add_settings({"foo.directories": "tests:fixtures"})
        config.commit()
        result = render("nonminimal.foo", {"name": "<b>fred</b>"}).replace("\r", "")
        self.assertEqual(result, text_("Hello, &lt;b&gt;fred&lt;/b&gt;!\n"))
        config.end()
...
class TestIntegrationNoDirectories(unittest.TestCase):
    ...
    def test_template_not_found(self):
        ...
        self.assertRaises(
            TemplateLookupException, render, "fixtures/helloworld_not_here.mak", {}
        )

    def test_template_default_escaping(self):
        from pyramid.renderers import render

        result = render("fixtures/nonminimal.mak", {"name": "<b>fred</b>"}).replace(
            "\r", ""
        )
        self.assertEqual(result, text_("Hello, &lt;b&gt;fred&lt;/b&gt;!\n"))
...
class TestPkgResourceTemplateLookup(unittest.TestCase):
    ...
    def get_fixturedir(self):
        ...
        return os.path.join(os.path.dirname(tests.__file__), "fixtures")
    ...
    def test_get_template_asset_spec_with_filesystem_checks(self):
        ...
        result = inst.get_template("tests:fixtures/helloworld.mak")
        self.assertFalse(result is None)
    ...
    def test_get_template_asset_spec_missing(self):
        ...
        self.assertRaises(
            TopLevelLookupException, inst.get_template, "tests:fixtures/notthere.mak"
        )
